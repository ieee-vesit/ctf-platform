...
const Logs = () => {
  ...
  const filteredLogs = useMemo(() => {
    return logsData.filter((log) => {
      const matchesSearch = log.teamName
        .toLowerCase()
        .includes(searchQuery.toLowerCase());
      const matchesCategory =
        selectedCategory === "All Categories" ||
        log.challengeCategory === selectedCategory;
      return matchesSearch && matchesCategory;
    });
  }, [searchQuery, selectedCategory]);
  ...
  const totalPages = Math.ceil(
    totalFilteredLogs / LOGS_PER_PAGE
  );
  const startIndex = (currentPage - 1) * LOGS_PER_PAGE;
  const endIndex = startIndex + LOGS_PER_PAGE;
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 text-sm bg-zinc-900 border border-zinc-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-yellow-400"
            />
          </div>
          ...
          {/* Refresh */}
          <button className="p-2.5 bg-zinc-900 border border-zinc-800 rounded-lg hover:bg-zinc-800">
            ...
            {currentData.length > 0 ? (
              currentData.map((log) => (
                ...
                >
                  ...
                  <div className="col-span-4 flex items-center gap-3">
                    <div
                      className={`w-8 h-8 rounded-full ${log.teamColor} flex items-center justify-center`}
                    >
          ...
          {currentData.map((log) => (
            ...
            >
              <div className="flex justify-between mb-3">
                <div className="flex items-center gap-3">
                  <div
                    className={`w-10 h-10 rounded-full ${log.teamColor} flex items-center justify-center`}
                  >
              ...
              <div className="pt-3 border-t border-zinc-800 text-sm text-gray-300">
                {log.challengeCategory}
              </div>
            </div>
          ))}
